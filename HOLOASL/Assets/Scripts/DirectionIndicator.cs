...
namespace HoloToolkit.Unity
{
    ...
    public class DirectionIndicator : MonoBehaviour
    {
        ...
        private GameObject InstantiateDirectionIndicator(GameObject directionIndicator)
        {
            ...
            Debug.Log(" DirectionalRenderer is : " + directionIndicatorRenderer);
            if (directionIndicatorRenderer == null)
            {
                //return null;

                directionIndicatorRenderer = indicator.AddComponent<MeshRenderer>();
                Debug.Log("Inside DirectionalRenderer is : " + directionIndicatorRenderer);
            ...
            Debug.Log(" indicator Material is :  " + indicatorMaterial + " color is : " + indicatorMaterial.color);
            return indicator;
        }
        ...
        public void Update()
        {
            ...
            if (isDirectionIndicatorVisible)
            {
                ...
                GetDirectionIndicatorPositionAndRotation(
                    ...
                    out position,
                    out rotation);
                //DirectionIndicatorObject.transform.localScale = new Vector3(0.4f, 0.4f, 0.4f);
            ...
            {
                //DirectionIndicatorObject.transform.localScale = new Vector3(0f, 0f, 0f);
        ...
        private bool IsTargetVisible(Camera mainCamera)
        {
            ...
            /*
            GameObject botObject = GameObject.FindGameObjectWithTag("BotTag");
            GameObject cursor = GameObject.Find("Cursor");
            BoxCollider botCollider = botObject.GetComponent<BoxCollider>();
            if (botCollider == null)
                return true;
            float thresholdX = botCollider.size.x;
            float thresholdY = botCollider.size.y;

            
            if (Mathf.Abs(cursor.transform.position.x - botObject.transform.position.x) > thresholdX ||
                Mathf.Abs(cursor.transform.position.y - botObject.transform.position.y) > thresholdY)
            {
                return false;
            }
            else
